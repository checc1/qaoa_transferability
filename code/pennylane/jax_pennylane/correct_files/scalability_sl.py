...
def transfer_scalability_sl(seed: int, graph: nx.Graph or list, graph_sorgent: nx.Graph, params_to_transfer: jnp.ndarray) -> list[float]:
    @jax.jit
    ...
    optax_optimizer = optax.adagrad(learning_rate=0.1)
    layer_optimized = np.array([params_to_transfer[opt_layers]])
    opt_state = optax_optimizer.init(layer_optimized)
    steps = 500
    prev_obj_val = obj_function_acceptor(params_to_transfer)
    num_occurrances = 0

    for i in range(steps):
        f = obj_function_acceptor(layer_optimized)
        if i == 0:
            print("f: ", f)
        if f != 0:
            grads = jax.grad(obj_function_acceptor)(layer_optimized)
            updates, opt_state = optax_optimizer.update(grads, opt_state)
            layer_optimized = optax.apply_updates(layer_optimized, updates)
            total_params1 = params_to_transfer
            total_params = total_params1.at[opt_layers].set(layer_optimized[0])
            print("TOTAL PARAMS\n:", total_params)
            current_obj_val = obj_function_acceptor(layer_optimized)
            # print("OBJ FUNC:\n", current_obj_val)
    ...
    counts_acceptor = acceptor_qnode_counts(layer_optimized, graph, edge=None)
...
def scalability() -> pd.DataFrame:
    ...
    while count_graph < count_graph_target:
        ...
        if nx.is_connected(graph_generator_acceptor):
            ...
            acceptor_ar, p_acceptor = transfer_scalability_sl(s, acceptor, graph_generator_acceptor, params_from_donor)
...
if __name__ == "__main__":
    ...
    dataset = scalability()
    dataset.to_json(save_path + f"/data_scalability_singleLayer{donor_node}_{acceptor_node}.json", orient="records", lines=True)
    #dataset.to_csv(
