...
donor_node = 12
acceptor_node = 14
p = float(sys.argv[1])  # Probability for edge generation
initial_seed = 349
total_seed = 50
save_path = "results/"
shots = 100_000
...
def scalability(p: float) -> pd.DataFrame:
    ...
    edges_acceptor = []
    s = 0
    graph_generator_donor = RandomGraph(donor_node, prob=p, seed=initial_seed)
    edges_donor = graph_generator_donor.number_of_edges()
    ...
    while count_graph < count_graph_target:
        print(f"Seed: {s}")
        graph_generator_acceptor = RandomGraph(acceptor_node, prob=p, seed=s)
        if nx.is_connected(graph_generator_acceptor):
            acceptor = list(graph_generator_acceptor.edges)
            acceptor_ar, _ = transfer_scalability(s, acceptor, graph_generator_acceptor, params_from_donor)
            ar_list_acceptors.append(acceptor_ar)
            edges_acceptor.append(graph_generator_acceptor.number_of_edges())
    ...
    df = pd.DataFrame({
        "Seed": seed_,
        "Edges donor": [edges_donor] * len(seed_),
...
if __name__ == "__main__":
    ...
    dataset.to_json(save_path + f"/data_scalability_{donor_node}_{acceptor_node}_Sameprob{p}.json", orient="records", lines=True)
    #dataset.to_csv(
