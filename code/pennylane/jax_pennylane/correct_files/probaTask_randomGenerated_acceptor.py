...
donor_node = 12
acceptor_node = 14
p = int(sys.argv[1])  # Probability for edge generation
initial_seed = 349
total_seed = 50
save_path = "results/"
shots = 100_000
...
def transfer_scalability(seed: float, graph: nx.Graph or list, graph_sorgent: nx.Graph, params_to_transfer: jnp.ndarray) -> list[float]:
    @jax.jit
...
def scalability(p_acceptor: int) -> pd.DataFrame:
    ...
    #s = 0
    p_donor = 0.6
    graph_generator_donor = RandomGraph(donor_node, prob=p_donor, seed=initial_seed)
    n_edges_donor = graph_generator_donor.number_of_edges()
    degree_donor = [val for (node, val) in sorted(graph_generator_donor.degree(), key=lambda pair: pair[0])]
    ...
    p_acceptor_normalized = p_acceptor / 10
    while count_graph < count_graph_target:
        print(f"Acceptor p: {p_acceptor_normalized}")
        graph_generator_acceptor = RandomGraph(acceptor_node, prob=p_acceptor_normalized, seed=count_graph)
        if nx.is_connected(graph_generator_acceptor):
            acceptor = list(graph_generator_acceptor.edges)
            acceptor_ar, _ = transfer_scalability(p_acceptor_normalized, acceptor, graph_generator_acceptor, params_from_donor)
            n_edges_acceptor = graph_generator_acceptor.number_of_edges()
            degree_acceptor = graph_generator_acceptor.degree()
            degree_per_node = [val for (node, val) in sorted(degree_acceptor, key=lambda pair: pair[0])]
            ar_list_acceptors.append(acceptor_ar)
            #probs.append(p_acceptor)
            edges_acceptor.append(n_edges_acceptor)
            d_acceptor.append(degree_per_node)
            count_graph += 1
            seed_.append(p_acceptor)
        p_acceptor_normalized += 0.01
    df = pd.DataFrame({
        "Seed": seed_,
        "Edges_donor": [n_edges_donor] * len(seed_),
        "Degree donor": [degree_donor] * len(seed_),
        "Ar_acceptor": ar_list_acceptors,
        #"Prob_acceptor": probs,
        "Ar_donor": [ar_donors] * len(seed_),
        "Degree acceptor": d_acceptor,
        "Edges acceptor": edges_acceptor
    })
    return df
...
if __name__ == "__main__":
    ...
    dataset = scalability(p)
    dataset.to_json(save_path + f"/data_scalability_{donor_node}_{acceptor_node}_fromAcceptor_prob{p, p+1}.json", orient="records", lines=True)
    #dataset.to_csv(
